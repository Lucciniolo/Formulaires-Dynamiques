...
class JFormer {
    ...
    function getHtml() {
        ...
        $previousButton->update('Précédent');
...
class JFormComponentAddress extends JFormComponent {
    ...
    function __construct($id, $label, $optionArray = array()) {
        ...
        if ($this->emptyValues === true) {
            $this->emptyValues = array('addressLine1' => 'Adresse', 'addressLine2' => 'Compléments d\'adresse', 'city' => 'Ville', 'Pays' => 'State / Province / Region', 'zip' => 'Postal / Zip Code');
        }
    ...
    function __toString() {
        ...
        if ($this->showSublabels) {
            $addressLine1Div->insert('<div class="jFormComponentSublabel"><p>Adresse</p></div>');
            $addressLine2Div->insert('<div class="jFormComponentSublabel"><p>Compléments d\'adresse</p></div>');
            $cityDiv->insert('<div class="jFormComponentSublabel"><p>Ville</p></div>');

            if ($this->unitedStatesOnly) {
                $stateDiv->insert('<div class="jFormComponentSublabel"><p>Pays</p></div>');
            } else {
                $stateDiv->insert('<div class="jFormComponentSublabel"><p>Région</p></div>');
            }

            if ($this->unitedStatesOnly) {
                $zipDiv->insert('<div class="jFormComponentSublabel"><p>Code postal</p></div>');
            } else {
                $zipDiv->insert('<div class="jFormComponentSublabel"><p>Code postal</p></div>');
            }

            $countryDiv->insert('<div class="jFormComponentSublabel"><p>Pays</p></div>');
        }
...
class JFormComponentName extends JFormComponent {
    ...
    function __construct($id, $label, $optionArray = array()) {
        ...
        if ($this->emptyValues === true) {
            $this->emptyValues = array('firstName' => 'Prénom', 'middleInitial' => 'M', 'lastName' => 'Nom');
        }
        //$this->mask = '';
        // Initialize the abstract FormComponent object
        $this->initialize($optionArray);
    ...
    function __toString() {
        ...
        if ($this->showSublabels) {
            $firstNameDiv->insert('<div class="jFormComponentSublabel"><p>Prénom</p></div>');
            $middleInitialDiv->insert('<div class="jFormComponentSublabel"><p>MI</p></div>');
            $lastNameDiv->insert('<div class="jFormComponentSublabel"><p>Nom</p></div>');
        }
